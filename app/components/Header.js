...
const Header = () => {
    ...
    const handleSearch = async (event) => {
        ...
        debounceTimeout.current = setTimeout(async () => {
            ...
            try {
                const response = await fetch(`http://apimanga.wocogeh.com/manga/v2/page/1/${query}/`)  // Memanggil API
    ...
    return (
        <>
            <div className="relative bg-white p-4 m-auto max-w-[500px]">
                ...
                <form>
                    <input
                        className="bg-slate-100 outline-none text-dark w-full p-4 rounded-lg transition-all focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 z-30"
                        type="text"
                ...
                {loading && (
                    <div className="mt-4 text-center text-gray-500">Sedang memuat...</div>
                )}
                            ...
                            <Link key={index} href={`/pages/detail/${komik.link}`}>
                                <li className="flex items-center gap-3 p-2 hover:bg-gray-100 rounded-md">
                                        ...
                                        <div className="text-sm">
                                            <h5 className="font-semibold">{komik.judul}</h5>
                                            <p className="text-gray-500">{komik.chapter}</p>
                                        </div>
                                </li>
                            </Link>
                            ))}
